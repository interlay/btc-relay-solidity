...
async function getBestBlockHeight(relay: Relay): Promise<number> {
  const {height} = await relay.getBestBlock();
  return height;
}
...
describe('Proofs', () => {
  ...
  it('should validate empty block coinbase', async () => {
    const header =
      '0x000000208eac45cbfb6620c53cbe8d20dd637f5e305118fa798f337d92210300000000009f3bf29af9132c4df7edb1d6f1f6306da12a28295d0918d2e424d27418b74d821aa9dc5ef0ff0f1ba5938e48';
    ...
    const txId = Buffer.from(
      '824db71874d224e4d218095d29282aa16d30f6f1d6b1edf74d2c13f99af23b9f',
      'hex'
    ).reverse();
  ...
  const testnet1 = {
    ...
    header:
      '0x00000020c29de51a684e5219b6cfe25f6999a31e05b5626a33478a8e6ed4869d000000005125f5cb99ebbfc5d2397d8282bd849aee26f984111230c8c42ee25dbd361e944d20d95effff001d97bebd41',
    headerHash:
      '0x000000000ee8388b0c4b935dec68824af4ad284dda2065d3eab526f207b17d8c',
    height: 1747715
  };
  ...
  it('should validate inclusion (testnet1)', async () => {
    ...
    const proof = testnet1.intermediateNodes
      .map((value) => Buffer.from(value, 'hex').reverse().toString('hex'))
      .join('');
    const txid = Buffer.from(testnet1.txId, 'hex').reverse().toString('hex');
  ...
  it('should validate inclusion (testnet2)', async () => {
    ...
    const proof = testnet2.intermediateNodes
      .map((value) => Buffer.from(value, 'hex').reverse().toString('hex'))
      .join('');
    const txid = Buffer.from(testnet2.txId, 'hex').reverse().toString('hex');
  ...
  const testnet3 = {
    ...
    header:
      '0x00e0ff27c57b44820f2629d53e7ed7f1cb147407b6b059b1d9c479414901000000000000b3d9797f1e11c29e5f955051b3ab9a20043777f6620c5ebe3a01955c25196f9349f6f15ef2a5011aa9b53fc8',
    headerHash:
      '0x00000000000000155b7e1e56cc21226981bb7bdf4c506f35c00aa01649321566',
    height: 1773152
  };
  ...
  it('should validate inclusion (testnet3)', async () => {
    ...
    const proof = testnet3.intermediateNodes
      .map((value) => Buffer.from(value, 'hex').reverse().toString('hex'))
      .join('');
    const txid = Buffer.from(testnet3.txId, 'hex').reverse().toString('hex');
